---
lab:
    title: 'Perform Data Analysis in Power BI (challenge version)'
    module: '8 - Perform Data Analysis in Power BI'
---


# **Perform Data Analysis in Power BI** (challenge version)
...
2. Add the following fields to the visual wells/areas:
...
3. Add the **Product \| Category** field to the **Filters On This Page** well/area. Filter by **Bikes**.

5. To animate the chart, at the bottom left corner, select **Play**.

	![Picture 41](Linked_image_Files/10-perform-data-analysis-in-power-bi-desktop_image19.png)

6. Watch the entire animation cycle from **FY2018 Q1** to **FY2020 Q4**.
...
7. When the animation stops, select one of the bubbles to reveal its tracking over time.

8. Hover the cursor over any bubble to reveal a tooltip describing the measure values for the reseller type at that point in time.

9. In the **Filters** pane, filter by **Clothing** only, and notice that it produces a very different result.
...
3. Add the **Date \| Year** field to the **Filters On This Page** well/area. Filter by two years: **FY2019** and **FY2020**.
...
5. Add also the **Product \| Category** field to the **Filters On This Page** well/area, and filter by **Bikes**.

6. Add a forecast from the **Analytics** pane.

	 ![Picture 20](Linked_image_Files/10-perform-data-analysis-in-power-bi-desktop_image26.png)

7. Expand the **Forecast** section.
...
8. Turn the **Forecast** option to **On**.

9. Configure the following forecast properties, then **Apply**:
...
10. In the line visual, notice that the forecast has extended one month beyond the history data.
...
11. In the **Filters** pane, filter by **Clothing** only, and notice that it produces a different result.
