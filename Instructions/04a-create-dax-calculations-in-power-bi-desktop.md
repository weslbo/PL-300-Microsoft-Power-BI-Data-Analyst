...
1. Open the **D:\PL300\Labs\04-create-dax-calculations-in-power-bi-desktop\Starter\Sales Analysis.pbix** file.
...
10. For the **Salesperson** table, set the description to: **Salesperson related to Sales**
...
1. Create a new **New Table** and enter the following formula:
...
2. Notice the column of date values.
...
3. At the bottom-left corner, in the status bar, notice the table statistics, confirming that 1826 rows of data have been generated, which represents five full years’ data.
...
1. Create a new calculated column, named **Year** and use the following formula:
...
4. Repeat the same steps to create the following two calculated columns for the **Date** table:
...
5. Switch to Report view, create a new report page and add a matrix visual. It should become like this:

	![Picture 20](Linked_image_Files/05-create-dax-calculations-in-power-bi-desktop_image27.png)

	*Notice that the years expand to months, and that the months are sorted alphabetically rather than chronologically.*

	*By default, text values sort alphabetically, numbers sort from smallest to largest, and dates sort from earliest to latest.*

12. To customize the **Month** field sort order, switch to Data view and add the **MonthKey** column to the **Date** table.
...
3. Create a hierarchy, named **Fiscal** from the **Date** table and add the following fields to it:
...
1. Mark the **Date** table as a data table (using the Date column)
...
4. Create a measure in the **Sales** table with the following measure definition:
...
9. Create the following five measures for the **Sales** table:
	...
	- Orders (tip: use DISTINCTCOUNT)

	- Order Lines (tip: use COUNTROWS)
...
3. Remove the **Target** field from the table visual.
...
10. Create the following two measures for the **Targets** table:

	- Variance (Tip: sales minus target)

	- Variance Margin (TIP: DIVIDE variance with target)
